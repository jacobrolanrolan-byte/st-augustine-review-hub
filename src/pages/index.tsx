...
import { Box, Container, Typography, Grid, Card, CardContent, Button, Chip, CircularProgress, TextField, Dialog, DialogTitle, DialogContent, DialogActions, Select, MenuItem, FormControl, InputLabel, Divider, List, ListItem, ListItemButton, ListItemIcon, ListItemText, InputAdornment, IconButton } from '@mui/material';
...
export default function Home() {
  ...
  // Direct UI Delete Action Handler
  const handleDeleteMaterial = async (id: string, downloadUrl: string) => {
    ...
    try {
      // 1. Extract file path from public URL to clean out storage asset
      ...
      // 2. Erase core data row reference trace out of materials table
      const { error } = await supabase.from('materials').delete().eq('id', id);
      if (error) throw error;

      fetchMaterials(); // Instantly refresh layout stream
    } catch (error: any) {
  ...
  // Combined Multi-Filter Operation Matrix (Subject Sort + Live Search Text Mapping)
  ...
  return (
    <Box sx={{ bgcolor: '#F8F9FA', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ display: 'flex', flexGrow: 1 }}>
        ...
        {/* Sidebar Directories Component */}
        <Box sx={{ width: '280px', borderRight: '1px solid #E0E0E0', bgcolor: '#FFFFFF', display: { xs: 'none', lg: 'block' }, p: 2 }}>
          <Typography variant="caption" sx={{ color: '#70757A', fontWeight: 600, px: 2, display: 'block', mb: 1, letterSpacing: '0.8px', textTransform: 'uppercase' }}>
            Course Directories
          </Typography>
          <List>
        ...
        {/* Primary Content Stream Workspace Canvas */}
        <Box sx={{ flexGrow: 1, p: { xs: 3, md: 4 } }}>
          <Container maxWidth="xl" disableGutters>
            
            {/* Top Toolbar Header Alignment containing Live Search Bar Block */}
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={4} flexWrap="wrap" gap={2}>
              ...
              {isAdmin && (
                <Button variant="contained" startIcon={<CloudUploadIcon />} onClick={() => setOpenAdminModal(true)} sx={{ bgcolor: '#1A73E8', boxShadow: 'none', textTransform: 'none', '&:hover': { bgcolor: '#1557B0', boxShadow: 'none' } }}>
            ...
            {filteredMaterials.length === 0 ? (
              <Box sx={{ p: 8, border: '1px dashed #DADCE0', borderRadius: '8px', textAlign: 'center', bgcolor: '#FFFFFF' }}>
                ...
                <Typography variant="body2" sx={{ color: '#70757A' }}>Try typing an alternate search parameter title sequence or tracking key phrase signature.</Typography>
              </Box>
            ) : (
              <Grid container spacing={3}>
                {filteredMaterials.map((item) => (
                  <Grid item xs={12} sm={6} md={4} key={item.id}>
                    <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: '#FFFFFF', border: '1px solid #E0E0E0', borderRadius: '8px', boxShadow: 'none', position: 'relative' }}>
                      <CardContent sx={{ p: 3, flexGrow: 1 }}>
                        <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={2}>
                          <Chip label={item.subject} size="small" sx={{ bgcolor: '#E8F0FE', color: '#1A73E8', fontWeight: 600, borderRadius: '4px', fontSize: '0.75rem' }} />
                          
                          {/* Admin-Exclusive Material Disposal Button */}
{isAdmin && (
  <IconButton 
    size="small" 
    color="error" 
  ...
  >
    <DeleteIcon fontSize="small" />
                        ...
                        <Typography variant="body2" sx={{ color: '#5F6368', fontSize: '0.85rem' }}>{item.description || "No supplemental details populated."}</Typography>
                      </CardContent>
                      <Divider />
                      <Box sx={{ px: 3, py: 2, bgcolor: '#F8F9FA', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Typography variant="caption" sx={{ color: '#70757A' }}>By: {item.uploaded_by?.split('@')[0]}</Typography>
                        <Button variant="text" endIcon={<OpenInNewIcon fontSize="small" />} href={item.download_url} target="_blank" sx={{ textTransform: 'none', color: '#1A73E8', fontWeight: 600, p: 0 }}>View</Button>
                      </Box>
                    </Card>
                  </Grid>
                ))}
      ...
      <Dialog open={openAdminModal} onClose={() => setOpenAdminModal(false)} fullWidth maxWidth="sm">
        ...
        <form onSubmit={handleUploadSubmit}>
          <DialogContent dividers sx={{ py: 3 }}>
            ...
            <Box sx={{ mt: 3, p: 3, border: '2px dashed #DADCE0', borderRadius: '4px', bgcolor: '#F8F9FA', textAlign: 'center' }}>
              ...
              <Typography variant="body2" sx={{ mt: 1.5, color: selectedFile ? '#386A20' : '#70757A', fontWeight: selectedFile ? 600 : 400 }}>
          ...
          <DialogActions sx={{ p: 2.5 }}>
            ...
            <Button type="submit" variant="contained" disabled={uploading} sx={{ bgcolor: '#1A73E8', textTransform: 'none', boxShadow: 'none' }}>
